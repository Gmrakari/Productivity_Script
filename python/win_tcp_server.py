# 2023-05-19 Windows tcp recv img
# Usage:
...
def tcp_server_start():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        # 绑定服务器地址和端口号
        server_address = ('172.20.10.4', 8000)
        ...
        while True:
            ...
            try:
                ...
                while True:
                    data = b''
                    while len(data) < 640 * 480 * 4 * 2 + 128 * 128 * 4 * 2 + 3 * 4:
                    ...
                    print(len(data))
                    # 解析数据
                    vis_img_buf_len = 640 * 480 * 4
                    ir_img_buf_len = 640 * 480 * 4
                    ...
                    vis_img_file_name = os.path.join(img_folder_current,
                                                     'vis_b_img_[prob{}]_[score{}]_[live_score{}].bmp'.format(prob, score, live_score))
                    cv2.imwrite(vis_img_file_name, vis_img)

                    ir_img_file_name = os.path.join(img_folder_current,
                                                    'ir_b_img_[prob{}]_[score{}]_[live_score{}].bmp'.format(prob, score, live_score))
                    cv2.imwrite(ir_img_file_name, ir_img)

                    vis_img_file_name = os.path.join(img_folder_current,
                                                     'vis_s_img_[prob{}]_[score{}]_[live_score{}].bmp'.format(prob, score, live_score))
                    cv2.imwrite(vis_img_file_name, vis_s_img)

                    ir_img_file_name = os.path.join(img_folder_current,
                                                    'ir_s_img_[prob{}]_[score{}]_[live_score{}].bmp'.format(prob, score, live_score))
                    cv2.imwrite(ir_img_file_name, ir_s_img)
